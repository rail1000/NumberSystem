...
public class DoBase64Conti extends HttpServlet {
    protected void doPost(HttpServletRequest request,
        ...
        try(PrintWriter out = response.getWriter();){
         ...
         try{
               ...
               String sqlQuery = "select path from imgConti where chec='0'";
              ...
              int i=0;
               //System.out.println(":"+);
              String sqlQuery1 = "select path from imgConti where chec='0'";
            ...
            for(int j = 0;j<imgUri.length;j++){
                ...
                try {
                    ...
                    System.out.println("strArray:"+strArray[2]);
                    System.out.println("strArray:"+strArray[3]);
                    String Bid = request.getParameter("Bid");
                     System.out.print("Bid:");
                     System.out.println(Bid);
                     
                    String sqlQuery2 = "select typeA,typeB,typeC,numBegin,numEnd from applyconti where id ="+Bid;
                    
                    rs2 = sql1.executeQuery(sqlQuery2);
                   
                    String sqlQuery3 = "select typeA,typeB,typeC,numBegin,numEnd from applyconti where id ="+Bid;
                   // rs3 = sql.executeQuery(sqlQuery3);
                    ...
                    try{
                        ...
                        int a = Integer.parseInt(rs2.getString(4));
                        int b = Integer.parseInt(rs2.getString(5));
                        number = new String[(b-a)+1];
                        for(int v=0;v<=b-a;v++){
                        
                        number[v] = rs2.getString(1)+rs2.getString(2)+rs2.getString(3)+(a+v);
                        t++;
                        }
                     System.out.println("number2222:"+number[0]);}
                    ...
                    */
                    ...
                    System.out.println("个数temp:"+temp);
                    if(temp == count){
                        String sqlUpdate = "update imgConti set chec=1,agree=1 where Bid ="+Bid;
                               sql.executeUpdate(sqlUpdate);
                               String sqlUpdate1 = "update applyConti set chec=2,agree=2 where id ="+Bid;
                               sql1.executeUpdate(sqlUpdate1);
                                String message = "校验成功";
                                //重定向
                                 response.sendRedirect("httpTestConti.jsp?message="+message);
                    
                    }else{
                          String sqlUpdate1 = "update imgConti set chec=1,agree=0 where Bid ="+Bid;
                                sql.executeUpdate(sqlUpdate1);
                                String sqlUpdate2 = "update applyConti set chec=1,agree=1 where id ="+Bid;
           ...
           //重定向
           // response.sendRedirect("httpTestConti.jsp");
        }
        catch(Exception ex){
